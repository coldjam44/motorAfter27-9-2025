<?php
// تحويل الصور الموجودة بامتداد إلى WebP وتحديث قاعدة البيانات
...
echo "<h2 class='text-center mb-4'>🌟 تحويل الصورة الموجودة إلى WebP</h2>";
...
// التحقق من أن نوع الصورة مدعوم
$allowedExtensions = ['jpg', 'jpeg', 'png', 'gif', 'bmp'];
if (!in_array($currentExtension, $allowedExtensions)) {
    echo "<div class='alert alert-danger'>";
    echo "<h5>❌ خطأ:</h5>";
    echo "<p>نوع الملف غير مدعوم: ." . htmlspecialchars($currentExtension) . "</p>";
    echo "<p>الأنواع المدعومة: " . implode(', ', $allowedExtensions) . "</p>";
    echo "</div>";
    echo "<a href='index.php' class='btn btn-secondary'>العودة للصفحة الرئيسية</a>";
    echo "</div></body></html>";
    exit;
...
echo "<p class='info'>📁 امتداد الملف الحالي: .$currentExtension</p>";
...
if ($webpCreated) {
    ...
    if (file_exists($webpFilePath)) {
        ...
        echo "<p class='info'>📊 حجم ملف WebP الجديد: " . number_format($webpSize / 1024, 2) . " KB</p>";
        ...
        try {
            if ($imageType == 'main') {
                ...
                if ($stmt->execute()) {
                    ...
                    echo "<p class='info'>📝 المسار القديم: $currentPath</p>";
                    echo "<p class='info'>📝 المسار الجديد: $webpDbPath</p>";
                    
                    // محاولة حذف الملف القديم (اختياري)
                    echo "<p class='info'>🗑️ محاولة حذف الملف الأصلي...</p>";
                    if (unlink($sourceFile)) {
                        echo "<p class='success'>✅ تم حذف الملف الأصلي: $foundFile</p>";
                    } else {
                        echo "<p class='warning'>⚠️ لم يتم حذف الملف الأصلي (ستحتاج لحذفه يدوياً)</p>";
                    }
                ...
                $sql = "UPDATE ad_images SET image = :new_path WHERE id = :image_id";
                $stmt = $pdo->prepare($sql);
                $stmt->bindParam(':new_path', $webpDbPath);
...
echo "<p>تم تحويل الصورة من .$currentExtension إلى .webp وتحديث قاعدة البيانات</p>";
echo "</div>";

echo "<div class='text-center'>";
echo "<a href='index.php' class='btn btn-primary me-2'>العودة للصفحة الرئيسية</a>";
echo "<a href='https://motorssooq.com/en/ad-details/" . htmlspecialchars($adId) . "' target='_blank' class='btn btn-secondary'>عرض الإعلان</a>";
echo "</div>";
